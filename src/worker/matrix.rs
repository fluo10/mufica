...
impl MatrixWorker{
    ...
    async fn login(data_dir: &Path, session_file: &Path) -> Result<Client> {
        ...
        let (client, client_session) = Self::build_client(data_dir).await?;
    ...
    async fn sync_matrix(client: Client,initial_sync_token: Option<String>, session_file: &Path,) -> Result<()> {
        ...
        loop {
            match client.sync_once(sync_settings.clone()).await {
                Ok(response) => {
                    ...
                    Self::persist_sync_token(session_file, response.next_batch).await?;
                    break;
                }
                Err(error) => {
        ...
        client.sync_with_result_callback(sync_settings, |sync_result| async move {
                ...
                Self::persist_sync_token(session_file, response.next_batch)
                    .await
